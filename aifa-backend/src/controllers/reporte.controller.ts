...
import { PdfExportService, ExcelExportService, CsvExportService } from '../services/exportacion';
import { DatosExportacion } from '../types/exportacion.types';

// Interfaces para tipado
interface AgrupacionResultado {
  [key: string]: number;
}

export class ReporteController {

  static getReporteGlobal = async (req: Request, res: Response) => {
    try {
  ...
  static exportarReporte = async (req: Request, res: Response) => {
    try {
      ...
      if (accesos.length === 0) {
        ...
        const totalAccesosPeriodo = await prisma.acceso.count({
          where: {
            fechaCreacion: {
              gte: inicio,
              lte: fin
      ...
      if (filtroId && filtroId !== '' && filtroId !== 'todos') {
        try {
          infoFiltro = await prisma.filtro.findUnique({
      ...
      if (usuarioId && usuarioId !== '' && usuarioId !== 'todos' && !registradoPor) {
        try {
          infoUsuario = await prisma.usuario.findUnique({
      ...
      if (filtroId && filtroId !== 'todos') tipoReporteFinal = 'por-filtro';
      if (usuarioId && usuarioId !== 'todos' && !registradoPor) tipoReporteFinal = 'por-usuario';
      if (registradoPor && registradoPor !== 'todos') tipoReporteFinal = 'por-registrador';
      ...
      switch (formato) {
        case 'pdf':
          resultado = await PdfExportService.generarReporte(datosExportacion);
          break;

        case 'excel':
          resultado = await ExcelExportService.generarReporte(datosExportacion);
          break;

        case 'csv':
          resultado = await CsvExportService.generarReporte(datosExportacion);
          break;
      ...
      await prisma.backupLog.create({
        data: {
          ...
          descripcion: `Reporte ${formato.toUpperCase()} - ${fechaInicio} a ${fechaFin}${filtroId && filtroId !== 'todos' ? ` - Filtro: ${infoFiltro?.nombre || filtroId}` : ''
            }${usuarioId && usuarioId !== 'todos' && !registradoPor ? ` - Usuario: ${infoUsuario?.nombre || usuarioId}` : ''
            }${registradoPor && registradoPor !== 'todos' ? ` - Registrador: ${registradoPor}` : ''
            }`,
  ...
  private static agruparPorArea(accesos: any[]): AgrupacionResultado {
    return accesos.reduce((acc: AgrupacionResultado, acceso) => {
  ...
  private static agruparPorMotivo(accesos: any[]): AgrupacionResultado {
    return accesos.reduce((acc: AgrupacionResultado, acceso) => {
  ...
  private static agruparPorFiltro(accesos: any[]): AgrupacionResultado {
    return accesos.reduce((acc: AgrupacionResultado, acceso) => {
  ...
  private static agruparPorTurno(accesos: any[]): AgrupacionResultado {
    return accesos.reduce((acc: AgrupacionResultado, acceso) => {
  ...
  private static agruparPorCreador(accesos: any[]): AgrupacionResultado {
    return accesos.reduce((acc: AgrupacionResultado, acceso) => {
  ...
  private static agruparPorDia(accesos: any[], inicio: Date, fin: Date): AgrupacionResultado {
    const resultado: AgrupacionResultado = {};
  ...
  static getReportePorFiltro = async (req: Request, res: Response) => {
    try {
  ...
  static getReportePorUsuario = async (req: Request, res: Response) => {
    try {
  ...
  static getEstadisticasRapidas = async (req: Request, res: Response) => {
    try {
      const { fechaInicio, fechaFin } = req.query;

      const inicio = fechaInicio ? new Date(fechaInicio as string) : new Date();
