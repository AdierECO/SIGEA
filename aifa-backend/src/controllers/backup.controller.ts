...
export class BackupController {
    static getEstadisticas = async (req: Request, res: Response) => {
        try {
            ...
            const ultimoBackup = await prisma.backupLog.findFirst({
                where: { tipo: 'EXPORTACION' },
    ...
    static exportarDB = async (req: Request, res: Response) => {
        try {
    ...
    private static async exportarJSON(incluirDatos: boolean, incluirEstructura: boolean, whereFecha: any) {
        const datos: any = {};
    ...
    private static async exportarSQL(incluirDatos: boolean, incluirEstructura: boolean, whereFecha: any) {
        try {
            let sqlContent = '';
    ...
    private static generarEstructuraSQL(): string {
        return `
-- =============================================
...
\n`;
    ...
    private static async generarDatosSQL(whereFecha: any): Promise<{ sql: string; totalRegistros: number }> {
        let sqlContent = '';
        let totalRegistros = 0;
        ...
        if (usuarios.length > 0) {
            ...
            const userValues = usuarios.map(user => 
                `(${user.id}, ${escapeSQL(user.email)}, ${escapeSQL(user.password)}, ${escapeSQL(user.nombre)}, ${escapeSQL(user.apellidos)}, ${escapeSQL(user.telefono)}, ${escapeSQL(user.rol)}, ${escapeSQL(user.estaActivo)}, ${user.intentosFallidos || 0}, ${escapeSQL(user.bloqueadoHasta)}, ${escapeSQL(user.ultimoAcceso)}, ${escapeSQL(user.fechaCreacion)}, ${escapeSQL(user.fechaActualizacion)}, ${user.filtroAsignadoId || 'NULL'})`
            ).join(',\n');
        ...
        if (identificaciones.length > 0) {
            ...
            const idValues = identificaciones.map(ident => 
                `(${ident.id}, ${escapeSQL(ident.tipo)}, ${escapeSQL(ident.numero)}, ${escapeSQL(ident.vigente)}, ${ident.filtro?.id || 'NULL'})`
            ).join(',\n');
        ...
        if (tias.length > 0) {
            ...
            const tiasValues = tias.map(tias => 
                `(${escapeSQL(tias.id)}, ${escapeSQL(tias.tipo)}, ${escapeSQL(tias.estado)}, ${tias.filtro?.id || 'NULL'}, ${escapeSQL(tias.fechaCreacion)}, ${escapeSQL(tias.fechaActualizacion)})`
            ).join(',\n');
        ...
        if (turnos.length > 0) {
            ...
            const turnoValues = turnos.map(turno => 
                `(${turno.id}, ${escapeSQL(turno.nombreTurno)}, ${escapeSQL(turno.horaInicio)}, ${escapeSQL(turno.horaFin)}, ${escapeSQL(turno.estaActivo)}, ${turno.creadoPor}, ${escapeSQL(turno.fechaCreacion)}, ${escapeSQL(turno.fechaActualizacion)})`
            ).join(',\n');
        ...
        if (filtros.length > 0) {
            ...
            const filtroValues = filtros.map(filtro => 
                `(${filtro.id}, ${escapeSQL(filtro.nombre)}, ${escapeSQL(filtro.descripcion)}, ${escapeSQL(filtro.ubicacion)}, ${filtro.cantidad || 'NULL'}, ${escapeSQL(filtro.estaActivo)}, ${filtro.usuarioCreadorId || 'NULL'})`
            ).join(',\n');
        ...
        if (turnoUsuario.length > 0) {
            ...
            const turnoUserValues = turnoUsuario.map(tu => 
                `(${tu.id}, ${tu.turnoId}, ${tu.usuarioId}, ${escapeSQL(tu.fechaAsignacion)})`
            ).join(',\n');
        ...
        if (backupLogs.length > 0) {
            ...
            const backupValues = backupLogs.map(backup => 
                `(${backup.id}, ${escapeSQL(backup.tipo)}, ${escapeSQL(backup.descripcion)}, ${backup.usuarioId}, ${escapeSQL(backup.detalles)}, ${escapeSQL(backup.fechaCreacion)})`
            ).join(',\n');
        ...
        if (auditorias.length > 0) {
            ...
            const auditoriaValues = auditorias.map(audit => 
                `(${audit.id}, ${escapeSQL(audit.tipo)}, ${escapeSQL(audit.accion)}, ${escapeSQL(audit.descripcion)}, ${audit.usuarioId || 'NULL'}, ${audit.accesoId || 'NULL'}, ${audit.turnoId || 'NULL'}, ${audit.identificacionId || 'NULL'}, ${audit.filtroId || 'NULL'}, ${audit.tiasId ? escapeSQL(audit.tiasId) : 'NULL'}, ${escapeSQL(audit.fechaCreacion)})`
            ).join(',\n');
    ...
    static importarDB = async (req: Request, res: Response) => {
        try {
            ...
            const getSafeExtension = (filename: string): string => {
                ...
                return parts.length > 1 ? (parts.pop() as string).toLowerCase() : 'desconocido';
            };
    ...
    private static async importarDesdeSQL(archivo: Express.Multer.File): Promise<any> {
        try {
    ...
    private static async importarDesdeJSON(archivo: Express.Multer.File, modo: string, conflictos: string) {
        try {
            ...
            if (datos.datos.identificaciones) {
                for (const identificacion of datos.datos.identificaciones) {
                    if (identificacion.numero) {
                        ...
                        await prisma.identificacion.create({
                            data: identificacion
                        });
                        registrosProcesados++;
                    }
    ...
    private static async crearBackupAutomatico(usuarioId: number) {
        try {
            ...
            await prisma.backupLog.create({
                data: {
                    ...
                    detalles: {
                        estadisticas: {
                            totalRegistros: Object.values(datos.datos).reduce((acc: number, curr: any) => 
    ...
    static getHistorial = async (req: Request, res: Response) => {
        try {
    ...
    static getEstadisticasBackups = async (req: Request, res: Response) => {
        try {
            const [totalExportaciones, totalImportaciones, totalBackupsAuto, ultimoBackup] = await Promise.all([
                prisma.backupLog.count({ where: { tipo: 'EXPORTACION' } }),
                prisma.backupLog.count({ where: { tipo: 'IMPORTACION' } }),
                prisma.backupLog.count({ where: { tipo: 'BACKUP_AUTO' } }),
