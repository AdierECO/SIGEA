...
export class AuditoriaController {
  static getLogsFiltro = async (req: Request, res: Response) => {
    try {
      const {
        page = 1,
        limit = 50,
      ...
      let filtroIdBusqueda: number | null = null;

      if (filtroId && filtroId !== 'todos') {
        filtroIdBusqueda = Number(filtroId);
      } else {
        const filtroUsuario = await prisma.usuario.findUnique({
      ...
      if (fechaInicio || fechaFin) {
        where.fechaCreacion = {};
        if (fechaInicio) where.fechaCreacion.gte = new Date(fechaInicio as string);
        if (fechaFin) {
      ...
      if (usuario) {
        where.usuario = {
          email: { contains: usuario as string, mode: 'insensitive' }
        };
      }
  ...
  static getDetalleAuditoria = async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
  ...
  static getEstadisticasFiltro = async (req: Request, res: Response) => {
    try {
