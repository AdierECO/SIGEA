...
export class ExcelExportService {
  ...
  private static agregarHojaResumen(workbook: ExcelJS.Workbook, datos: DatosExportacion): void {
  ...
  private static agregarHojaDetalles(workbook: ExcelJS.Workbook, datos: DatosExportacion): void {
    const { accesos } = datos;
    ...
    detailsSheet.columns.forEach((column: Partial<ExcelJS.Column>) => {
  ...
  private static agregarHojaEstadisticas(workbook: ExcelJS.Workbook, datos: DatosExportacion): void {
    const { accesos } = datos;
    ...
    Object.entries(areas).forEach(([area, count]) => {
      ...
      statsSheet.getCell(`B${rowIndex}`).value = count as number;
      rowIndex++;
    });
  }

  private static calcularTiempoPermanencia(entrada: string, salida: string): string {
