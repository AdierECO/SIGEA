...
class UsuarioController {
    ...
    static createUsuario = async (req, res) => {
        try {
            ...
            const existeUsuario = await data_1.prisma.usuario.findUnique({
            ...
            const hashedPassword = await config_1.bcryptjsAdapter.hash(data.password);
            const usuario = await data_1.prisma.usuario.create({
                data: {
                    ...data,
    ...
    static updateUsuario = async (req, res) => {
        try {
            const { id } = req.params;
            const data = req.body;
            const usuarioActual = req.user; // Usuario que hace la petición
            // Buscar usuario existente
            const usuarioExistente = await data_1.prisma.usuario.findUnique({
                where: { id: Number(id) },
                include: {
                    filtroAsignado: {
                        select: {
            ...
            if (data.email && data.email !== usuarioExistente.email) {
                const emailExistente = await data_1.prisma.usuario.findUnique({
                    where: { email: data.email }
                });
                if (emailExistente) {
            ...
            if (data.password) {
                data.password = await config_1.bcryptjsAdapter.hash(data.password);
            }
    ...
    static deleteUsuario = async (req, res) => {
        try {
            ...
            await data_1.prisma.usuario.delete({
                where: { id: Number(id) }
            });
            await data_1.prisma.auditoria.create({
                data: {
