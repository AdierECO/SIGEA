...
class TurnoController {
    ...
    static getTurnos = async (req, res) => {
        try {
            const turnos = await data_1.prisma.turno.findMany({
                include: {
    ...
    static getTurnoById = async (req, res) => {
        try {
            const { id } = req.params;
            const turno = await data_1.prisma.turno.findUnique({
                where: { id: Number(id) },
                include: {
                    creador: { select: { nombre: true, apellidos: true } },
                    usuarios: { include: { usuario: { select: { nombre: true, apellidos: true } } } },
    ...
    static createTurno = async (req, res) => {
        try {
            const data = req.body;
            const turno = await data_1.prisma.turno.create({
                data: {
    ...
    static updateTurno = async (req, res) => {
        try {
            const { id } = req.params;
            const data = req.body;
            const turnoExistente = await data_1.prisma.turno.findUnique({
                where: { id: Number(id) },
                include: {
                    usuarios: true
                }
            ...
            if (data.estaActivo === false && turnoExistente.estaActivo === true) {
                const result = await data_1.prisma.$transaction(async (tx) => {
            ...
            const turno = await data_1.prisma.turno.update({
                where: { id: Number(id) },
                data,
                include: {
                    creador: { select: { nombre: true, apellidos: true } },
    ...
    static asignarTurno = async (req, res) => {
        try {
            const { id } = req.params;
            ...
            const asignaciones = await data_1.prisma.turnoUsuario.createMany({
    ...
    static deleteTurno = async (req, res) => {
        try {
            const { id } = req.params;
            const turnoExistente = await data_1.prisma.turno.findUnique({
                where: { id: Number(id) },
            ...
            await data_1.prisma.$transaction(async (tx) => {
    ...
    static getTurnosActivos = async (req, res) => {
        try {
            const turnos = await data_1.prisma.turno.findMany({
    ...
    static desasignarTurno = async (req, res) => {
        try {
            const { id } = req.params;
            const { usuarioIds } = req.body;
            const turno = await data_1.prisma.turno.findUnique({
            ...
            await data_1.prisma.turnoUsuario.deleteMany({
                where: {
                    turnoId: Number(id),
    ...
    static desasignarUsuarioTurno = async (req, res) => {
        try {
            const { id } = req.params;
            ...
            const usuarioExists = await data_1.prisma.usuario.findUnique({
            ...
            const asignacionExists = await data_1.prisma.turnoUsuario.findUnique({
                where: {
                    turnoId_usuarioId: {
                        turnoId: Number(id),
            ...
            await data_1.prisma.turnoUsuario.delete({
                where: {
                    turnoId_usuarioId: {
                        turnoId: Number(id),
                        usuarioId: Number(usuarioId)
